...
void TaskHoleParameters::threadTypeChanged(int index)
{
    ...
    // ThreadSizeString has either the form "M3x0.35" or just "M3"
    // check if the string up to the 'x' is exists in the new list
    ThreadSizeString = ThreadSizeString.left(ThreadSizeString.indexOf("x") > -1 ? 
                                            ThreadSizeString.indexOf("x") : 
                                            ThreadSizeString.length());

    // Search for the string in the combobox and set the current index if found
    if (int idx = ui->ThreadSize->findText(ThreadSizeString, Qt::MatchContains); idx > -1) {
        ui->ThreadSize->setCurrentIndex(idx);
    }
